...
TIME_DAT=${RAW}/boot-serial-runc-wasm-time.dat
...
start_time=$(date +%s.%N)
    ...
    crictl -r unix:///run/containerd/containerd.sock run --runtime="runc" --no-pull $workdir/json/container/container_$i.json $workdir/json/pod/pod_$i.json
